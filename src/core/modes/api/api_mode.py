...
class APIMode:
    ...
    def __init__(self, config: APIModeConfig):
        ...
        try:
            ...
            self.logger.info(f"API mode initialized with endpoint: {config.api_url}{config.endpoint}")
    ...
    def analyze(self, prompt: str, **kwargs) -> AnalysisResult:
        ...
        with ContextTimer(self.performance, "api_analysis") as timer:
            ...
            if len(prompt.encode('utf-8')) > self.config.max_request_size:
                raise APIError(f"Prompt size exceeds maximum allowed size of {self.config.max_request_size} bytes")
        ...
        with ContextTimer(self.performance, "api_batch_analysis"):
            ...
            if hasattr(self.analyzer, 'analyze_batch'):
                try:
                    ...
                    self.logger.warning(f"Batch API analysis failed, falling back to individual calls: {e}")
