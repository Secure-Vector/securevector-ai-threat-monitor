...
class SmartRouter:
    ...
    def _make_routing_decision(self, risk_score: int, confidence: float, 
                              api_available: bool, local_accuracy: float,
                              prompt_complexity: float, **kwargs) -> bool:
    ...
    def _is_api_performing_well(self) -> bool:
        ...
        recent_failures = [
            failure_time for failure_time in self.api_performance["recent_failures"]
    ...
    def _assess_prompt_complexity(self, prompt: str) -> float:
        ...
        technical_keywords = [
            'system', 'database', 'api', 'code', 'script', 'algorithm',
            'configuration', 'authentication', 'authorization', 'encryption'
        ]
        technical_count = sum(1 for keyword in technical_keywords if keyword.lower() in prompt.lower())
    ...
    def _record_routing_decision(self, decision: bool, local_result: AnalysisResult, 
                               prompt: str) -> None:
    ...
    def update_api_performance(self, response_time_ms: float, success: bool) -> None:
        ...
        self.api_performance["avg_response_time"] = (
            ...
            (1 - alpha) * self.api_performance["avg_response_time"]
        )
        ...
        self.api_performance["success_rate"] = (
            alpha * success_value + 
            (1 - alpha) * self.api_performance["success_rate"]
        )
    ...
    def _adapt_threshold(self) -> None:
        ...
        local_accuracy = self._get_local_accuracy()
        api_accuracy = (
            self.accuracy_tracking["api_enhanced"]["correct"] / 
            max(self.accuracy_tracking["api_enhanced"]["total"], 1)
        )
