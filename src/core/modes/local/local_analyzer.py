...
from utils.security import (
    safe_regex_compile, safe_regex_search, RegexSecurityError, RegexTimeoutError,
    secure_path_join, validate_file_path, secure_file_glob, PathTraversalError
)
        ...
        try:
            ...
            try:
                ...
                if not rules_path.exists():
                    ...
                    bundled_path = Path(__file__).parent.parent.parent.parent.parent / "rules" / "bundled"
                    bundled_path = bundled_path.resolve()

                    if bundled_path.exists():
                        rules_path = bundled_path
                        self.logger.warning(f"Rules path not found, using bundled rules: {bundled_path}")
                    else:
                ...
                try:
                    ...
                    with open(rule_file, 'r', encoding='utf-8') as f:
                        ...
                        if 'rules' in rule_data:
                            pattern_count = sum(len(rule_entry.get('rule', {}).get('detection', [])) for rule_entry in rule_data['rules'])
                        else:
                    ...
                    self.logger.error(f"Security violation - path traversal in rule file {rule_file}: {e}")
                    continue
    ...
    def _compile_patterns(self, rule_name: str, rule_data: Dict) -> None:
            ...
            for rule_entry in rule_data['rules']:
                ...
                for detection in rule.get('detection', []):
                    ...
                    try:
                        ...
                        self.logger.warning(f"Unsafe regex pattern in {rule_id}: {pattern_str} - {e}")
                        continue
                    except RegexTimeoutError as e:
                        self.logger.warning(f"Regex compilation timeout in {rule_id}: {pattern_str} - {e}")
                        continue
                    except re.error as e:
                        self.logger.warning(f"Invalid regex pattern in {rule_id}: {pattern_str} - {e}")
                        continue
        ...
        # Handle old format for backward compatibility
            ...
            for pattern_info in rule_data.get('patterns', []):
                ...
                try:
                    # Use safe regex compilation with ReDoS protection
                    compiled_pattern = safe_regex_compile(pattern_str, re.IGNORECASE | re.MULTILINE, timeout=2.0)
                    compiled_patterns.append({
                        'compiled': compiled_pattern,
                        'original': pattern_str,
                    ...
                    self.logger.warning(f"Invalid regex pattern in {rule_name}: {pattern_str} - {e}")
                    continue
        ...
        for rule_name, rule_data in self.rules.items():
            ...
            # Handle old format for backward compatibility
                ...
                for pattern_info in rule_data['patterns']:
                    ...
                    try:
                        # Use safe regex compilation and search
                        compiled_pattern = safe_regex_compile(pattern_str, re.IGNORECASE | re.MULTILINE, timeout=1.0)
                        match = safe_regex_search(compiled_pattern, prompt, timeout=0.1)
                        if match:
                            detection = ThreatDetection(
    ...
    def get_pattern_count(self) -> int:
        ...
        for rule_data in self.rules.values():
            if 'rules' in rule_data:
                ...
                total += sum(len(rule_entry.get('rule', {}).get('detection', [])) for rule_entry in rule_data['rules'])
            else:
