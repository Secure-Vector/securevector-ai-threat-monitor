...
from ai_threat_monitor.models.config_models import LogLevel
...
class SecurityLogger:
    ...
    def log_threat(self, threat_type: str, risk_score: int, description: str,
                   analysis_time_ms: float, action: str = "detected") -> None:
    ...
    def log_clean_request(self, analysis_time_ms: float, method: str = "local") -> None:
        ...
        self.logger.info(
            f"✅ Request analyzed (Clean - {analysis_time_ms:.1f}ms, {method})"
        )
    ...
    def log_performance(self, operation: str, duration_ms: float, 
        ...
        self.logger.debug(
            f"{status} {operation} completed in {duration_ms:.1f}ms"
        )
    
    def log_mode_switch(self, from_mode: str, to_mode: str, reason: str) -> None:
        """Log mode switching"""
        self.logger.info(
            f"🔄 Mode switched: {from_mode} → {to_mode} ({reason})"
        )
    
    def log_api_call(self, endpoint: str, response_time_ms: float, 
                     status_code: int) -> None:
        """Log API calls"""
        status_emoji = "✅" if 200 <= status_code < 300 else "❌"
        self.logger.debug(
            f"{status_emoji} API {endpoint}: {status_code} "
    ...
    def log_rule_load(self, rule_file: str, rule_count: int, 
                      load_time_ms: float) -> None:
        """Log rule loading"""
        self.logger.info(
            f"📋 Loaded {rule_count} rules from {rule_file} "
            f"({load_time_ms:.1f}ms)"
        )
    
    def log_config_change(self, setting: str, old_value: str, 
                         new_value: str) -> None:
        """Log configuration changes"""
        self.logger.info(
            f"⚙️ Config changed: {setting} = {old_value} → {new_value}"
        )
    ...
    def print_session_summary(self) -> None:
        ...
        if self.session_stats['requests'] > 0:
            threat_rate = (self.session_stats['threats'] / 
                          self.session_stats['requests'] * 100)
