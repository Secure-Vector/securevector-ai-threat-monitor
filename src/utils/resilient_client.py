...
from utils.exceptions import (
    APIError, AuthenticationError, RateLimitError, CircuitBreakerError,
    ErrorCode, ConfigurationError
)
from utils.retry import CircuitBreaker, RetryConfig, with_retry, with_async_retry
from utils.telemetry import record_event, debug_log
...
class NetworkResilienceManager:
    ...
    def wrap_async_client_method(self, method: Callable, fallback_method: Optional[Callable] = None):
    ...
    def _execute_with_fallback(self, 
        ...
        try:
            ...
            record_event('circuit_breaker_triggered', 'resilience.network', {
    ...
    async def _execute_async_with_fallback(self,
                                         primary_method: Callable,
                                         fallback_method: Optional[Callable],
                                         *args, **kwargs) -> Any:
    ...
    def _handle_network_failure(self, error: Exception) -> None:
        ...
        record_event('network_failure', 'resilience.network', {
        ...
        if (self._consecutive_failures >= self.max_consecutive_failures and 
            not self._permanent_fallback):
            self._permanent_fallback = True
            self.logger.warning(
                f"Enabling permanent fallback after {self._consecutive_failures} consecutive failures"
            )
            record_event('permanent_fallback_enabled', 'resilience.network', {
                'consecutive_failures': self._consecutive_failures
            })
    ...
    def _execute_fallback(self, fallback_method: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            record_event('fallback_executed', 'resilience.network', {
                'method': fallback_method.__name__,
                'success': True
            })
            ...
            record_event('fallback_failed', 'resilience.network', {
                'method': fallback_method.__name__,
                'error': str(e)
            })
