...
from ai_threat_monitor.models.config_models import SDKConfig, OperationMode, LogLevel
...
class AutoConfigurator:
    ...
    def _is_running_in_docker(self) -> bool:
        ...
        return (
            ...
            any('docker' in line for line in open('/proc/self/cgroup', 'r').readlines())
        )
    ...
    def get_recommendations(self, config: SDKConfig) -> List[str]:
        ...
        if self._environment_info.get('cpu_count', 1) >= 4:
            recommendations.append("Consider using async client for better performance on multi-core systems")
        ...
        if config.mode != OperationMode.LOCAL and not self._environment_info.get('has_network', True):
...
def create_zero_config_client(mode: Optional[str] = None, 
                             api_key: Optional[str] = None,
                             **kwargs):
    """
    ...
    config = configurator.create_optimal_config(
        user_mode=operation_mode,
        api_key=api_key,
        **kwargs
    )
...
async def create_zero_config_async_client(mode: Optional[str] = None,
                                        api_key: Optional[str] = None,
                                        **kwargs):
