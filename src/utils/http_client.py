...
from utils.exceptions import APIError, RateLimitError, AuthenticationError, ErrorCode
from utils.retry import RetryConfig, with_retry, with_async_retry
from utils.telemetry import record_metric, debug_log
...
class HTTPConnectionPool:
    ...
    def _convert_exception(self, exc: Exception) -> Exception:
        ...
        if 'ssl' in str(exc).lower() or 'certificate' in str(exc).lower():
            return APIError(
                f"SSL/TLS error: {str(exc)}",
                error_code=ErrorCode.API_SSL_ERROR
            )
    ...
    def _update_metrics(self, url: str, request_time: float, status_code: int, error: bool = False):
        ...
        # Record telemetry
        record_metric('http.request.duration', request_time * 1000, 'ms', {
            'hostname': hostname,
            'status_code': str(status_code),
            'error': str(error)
        })
        
        debug_log(f"HTTP request completed", 
                 url=url, duration_ms=request_time * 1000, 
...
def get_http_client(base_url: Optional[str] = None,
                   pool_config: Optional[ConnectionPoolConfig] = None) -> PooledHTTPClient:
    """Get or create global HTTP client"""
    global _global_http_client
    
    with _client_lock:
        if _global_http_client is None:
            _global_http_client = PooledHTTPClient(
                base_url=base_url,
...
@asynccontextmanager
async def pooled_http_client(base_url: Optional[str] = None,
                           pool_config: Optional[ConnectionPoolConfig] = None):
