...
def assert_risk_score(result: AnalysisResult, 
...
def assert_threat_types(result: AnalysisResult,
    ...
    if exact_match:
        if set(actual_types) != set(expected_types):
            error_msg = message or (
                f"Expected exact threat types {expected_types}, "
                f"got {actual_types}"
            )
            raise AssertionError(error_msg)
    else:
        ...
        if missing_types:
            error_msg = message or (
                f"Missing expected threat types: {list(missing_types)}. "
                f"Got: {actual_types}"
            )
            raise AssertionError(error_msg)


def assert_confidence(result: AnalysisResult,
    ...
    if min_confidence is not None and actual_confidence < min_confidence:
        error_msg = message or (
            f"Confidence {actual_confidence} is below minimum {min_confidence}"
        )
        raise AssertionError(error_msg)
    
    if max_confidence is not None and actual_confidence > max_confidence:
        error_msg = message or (
            f"Confidence {actual_confidence} is above maximum {max_confidence}"
        )
        raise AssertionError(error_msg)


def assert_analysis_time(result: AnalysisResult,
    ...
    if max_time_ms is not None and actual_time > max_time_ms:
        error_msg = message or (
            f"Analysis time {actual_time}ms exceeds maximum {max_time_ms}ms"
        )
        raise AssertionError(error_msg)
    
    if min_time_ms is not None and actual_time < min_time_ms:
        error_msg = message or (
            f"Analysis time {actual_time}ms is below minimum {min_time_ms}ms"
        )
        raise AssertionError(error_msg)


def assert_batch_results(results: List[AnalysisResult],
...
def assert_detection_method(result: AnalysisResult,
                           expected_method: str,
                           message: Optional[str] = None):
    """
...
def assert_has_detections(result: AnalysisResult,
