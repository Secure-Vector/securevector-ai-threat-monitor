...
from .models.policy_models import SecurityPolicy, PolicyAction
from .types import (
...
def _constant_time_string_compare(s1: str, s2: str) -> bool:
    ...
    if len(s1) != len(s2):
        ...
        dummy_comparison = hmac.compare_digest(s1.ljust(max(len(s1), len(s2))), 
                                              s2.ljust(max(len(s1), len(s2))))
        return False
...
class SecureVectorClient:
    ...
    def analyze(self, prompt: str, **kwargs) -> AnalysisResult:
        ...
        with trace_operation('analyze_prompt', prompt_length=len(prompt) if prompt else 0) as operation_id:
            try:
                ...
                if policy_action == PolicyAction.BLOCK or (
                    ...
                    raise SecurityException(
                        f"Threat detected: {result.summary}",
                        result=result,
                ...
                self.logger.error(f"Analysis failed: {type(e).__name__}: {_sanitize_error_for_response(e, include_details=True)}")
    ...
    def get_stats(self) -> StatisticsDict:
        ...
        stats.update({
            ...
            "threat_rate": (
                stats["threats_detected"] / max(stats["total_requests"], 1) * 100
            ),
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        stats = self.get_stats()
        self.logger.info(f"Session complete: {stats['total_requests']} requests, "
