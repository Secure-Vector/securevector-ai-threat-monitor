...
from .testing import MockSecureVectorClient, MockBehavior, create_test_prompts
...
from utils.telemetry import get_telemetry_collector, TelemetryCollector
from utils.retry import RetryConfig, API_RETRY_CONFIG
...
class EnhancedCLI:
    ...
    def create_parser(self) -> argparse.ArgumentParser:
        ...
        # Global options
        parser.add_argument('--version', action='version', version='SecureVector Enhanced CLI 1.0.0')
        parser.add_argument('--debug', action='store_true', help='Enable debug mode')
        parser.add_argument('--telemetry', action='store_true', help='Enable telemetry collection')
        parser.add_argument('--output', '-o', choices=['json', 'yaml', 'csv', 'table'], 
    ...
    def _add_analysis_commands(self, subparsers):
        ...
        analyze_parser.add_argument('--mode', choices=['local', 'api', 'hybrid', 'auto'], 
        ...
        analyze_parser.add_argument('--async', dest='use_async', action='store_true', 
                                   help='Use async processing')
        analyze_parser.add_argument('--concurrent', type=int, default=5, 
                                   help='Max concurrent requests for async')
        
        # Interactive analyze command
        interactive_parser = subparsers.add_parser('interactive', 
                                                  help='Interactive analysis session')
        interactive_parser.add_argument('--mode', choices=['local', 'api', 'hybrid', 'auto'], 
                                       default='auto', help='Analysis mode')
    ...
    def _add_testing_commands(self, subparsers):
        ...
        test_parser.add_argument('--scenarios', nargs='+', 
                               ...
                               default=['mixed'], help='Test scenarios')
        ...
        generate_parser.add_argument('--scenario', choices=['safe', 'threat', 'mixed'], default='mixed')
        ...
        mock_parser.add_argument('--threat-rate', type=float, default=0.3, 
                               help='Simulated threat detection rate')
    ...
    def _add_streaming_commands(self, subparsers):
        ...
        stream_parser.add_argument('--file', '-f', type=str, required=True, 
                                  help='Large file to analyze')
        stream_parser.add_argument('--chunk-size', type=int, default=8192, 
                                  help='Chunk size in characters')
        stream_parser.add_argument('--overlap', type=int, default=256, 
                                  help='Overlap between chunks')
        stream_parser.add_argument('--concurrent', type=int, default=5, 
                                  help='Max concurrent chunks')
        stream_parser.add_argument('--strategy', choices=['max_risk', 'average', 'weighted'], 
                                  default='max_risk', help='Aggregation strategy')
        stream_parser.add_argument('--real-time', action='store_true', 
                                  help='Real-time streaming mode')
    
    def _add_benchmark_commands(self, subparsers):
        """Add benchmarking commands"""
        bench_parser = subparsers.add_parser('benchmark', help='Run performance benchmarks')
        bench_parser.add_argument('--mode', choices=['local', 'api', 'hybrid', 'all'], 
                                 default='all', help='Modes to benchmark')
        bench_parser.add_argument('--iterations', type=int, default=1000, 
                                 help='Number of iterations')
        bench_parser.add_argument('--concurrent', type=int, default=10, 
...
For detailed documentation: https://docs.securevector.dev/cli
    ...
    def _output_analysis_results(self, results, prompts, format_type, processing_time):
        ...
        if format_type == 'json':
            ...
            writer.writerow(['prompt', 'is_threat', 'risk_score', 'confidence', 
                           'threat_types', 'analysis_time_ms', 'summary'])
            
            for prompt, result in zip(prompts, results):
                writer.writerow([
                    prompt, result.is_threat, result.risk_score, result.confidence,
                    ';'.join(result.threat_types) if result.threat_types else '',
                    result.analysis_time_ms, result.summary
                ])
            ...
            for i, (prompt, result) in enumerate(zip(prompts, results), 1):
                status = "🔴 THREAT" if result.is_threat else "🟢 SAFE"
                print(f"{i:3d}. {status} | Risk: {result.risk_score:3d} | "
                      f"Confidence: {result.confidence:.1%} | "
            ...
            print(f"Summary: {threats}/{len(results)} threats ({threats/len(results)*100:.1f}%), "
