...
import time
from typing import (
    AsyncGenerator, AsyncIterator, Generator, Iterator, 
    List, Dict, Any, Optional, Union, Callable, Protocol
)
...
from .types import BaseSecureVectorClient, AsyncBaseSecureVectorClient
from utils.telemetry import trace_operation, record_metric
...
class StreamingAnalyzer:
    ...
    def analyze_stream(self, 
                      text: str,
                      **kwargs) -> Generator[StreamAnalysisResult, None, AnalysisResult]:
        """
        ...
        with trace_operation('stream_analysis', text_length=len(text)) as operation_id:
            ...
            record_metric('stream.total_processing_time', 
                         self._processing_stats["total_processing_time"], 'ms')
            
            return aggregated_result
    
    async def analyze_stream_async(self,
                                  text: str,
                                  **kwargs) -> AsyncGenerator[StreamAnalysisResult, AnalysisResult]:
        """
        ...
        with trace_operation('async_stream_analysis', text_length=len(text)) as operation_id:
            ...
            async def process_chunk_with_semaphore(chunk: StreamChunk, index: int) -> StreamAnalysisResult:
                async with semaphore:
                    start_time = time.time()
                    
                    try:
                        # Assume async client
                        if hasattr(self.client, 'analyze') and asyncio.iscoroutinefunction(self.client.analyze):
...
def analyze_large_text(client: BaseSecureVectorClient, 
                      text: str,
                      chunk_size: int = 8192,
...
async def analyze_large_text_async(client: AsyncBaseSecureVectorClient,
                                  text: str,
                                  chunk_size: int = 8192,
                                  **kwargs) -> AnalysisResult:
