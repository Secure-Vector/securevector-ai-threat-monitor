...
class CloudProxyService:
    ...
    async def analyze_output(
        ...
        *,
        ...
        """
        Proxy an LLM-output scan to the cloud /analyze/output endpoint.

        Distinct from analyze() which hits /analyze (input prompts). Output
        scans look for data leakage patterns the input-scan ruleset doesn't
        cover (PII in responses, credit cards, API keys leaked by the model,
        injection artifacts echoed back) and surface differently on the
        cloud dashboard's Output Analysis page.
        ...
        Returns a dict shaped like analyze() so analyze.py can treat input
        and output scans uniformly at the edge.
        """
        api_key = get_api_key()
        if not api_key:
            raise CloudProxyError("API key not configured")
