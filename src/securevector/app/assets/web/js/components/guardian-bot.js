...
const GuardianBot = {
    ...
    el(opts = {}) {
        ...
        wrap.innerHTML =
            ...
            // visor slot
            '<rect class="gb-visor" x="20" y="20" width="24" height="12" rx="6"/>' +
            // idle/scan eyes (dots that blink)
            '<circle class="gb-eye gb-eye-l" cx="27" cy="26" r="2.2"/>' +
            '<circle class="gb-eye gb-eye-r" cx="37" cy="26" r="2.2"/>' +
            // ok eyes (happy arcs, only in ok state)
            '<path class="gb-happy" d="M24.5 26.5q2.5 -3 5 0"/>' +
            '<path class="gb-happy" d="M34.5 26.5q2.5 -3 5 0"/>' +
...
.sv-gbot .gb-visor { stroke: var(--text-secondary, #aeb7c2); stroke-width: 2;
  fill: color-mix(in srgb, var(--accent-primary, #5eadb8) 10%, transparent); }
.sv-gbot .gb-tip { fill: var(--accent-primary, #5eadb8); }
.sv-gbot .gb-eye { fill: var(--accent-primary, #5eadb8); }
.sv-gbot .gb-happy { stroke: var(--accent-primary, #5eadb8); stroke-width: 2.4; }
...
.sv-gbot-scan .gb-eye { display: none; }
...
@media (prefers-reduced-motion: no-preference) {
  ...
  .sv-gbot-idle .gb-eye { animation: gb-blink 5.2s infinite; transform-origin: center; transform-box: fill-box; }
...
@keyframes gb-blink { 0%, 91%, 100% { transform: scaleY(1); } 94% { transform: scaleY(0.1); } 97% { transform: scaleY(1); } }
