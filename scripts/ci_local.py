...
def main():
    ...
    if not run_command(
        "black --check --diff src/ tests/ benchmarks/ scripts/",
    ...
    if not run_command(
        "isort --check-only --diff src/ tests/ benchmarks/ scripts/",
    ...
    # Linting
    if not run_command(
        "flake8 src/ tests/ benchmarks/ scripts/ --max-line-length=100 --extend-ignore=E203,W503",
        "Linting (flake8)",
    ):
        return 1
    ...
    # Type checking
    run_command(
        "mypy src/ --ignore-missing-imports", "Type checking (mypy)", continue_on_error=True
    )
    ...
    # Unit tests
    if not run_command(
        'pytest tests/ -v --cov=src --cov-report=term-missing -m "not benchmark"', "Unit tests"
    ):
        return 1
