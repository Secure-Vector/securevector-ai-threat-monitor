...
def validate_local_mode():
    ...
    try:
        ...
        try:
            ...
            print(
                f"  ✅ Threat prompt: Risk {result.risk_score}/100, {result.analysis_time_ms:.1f}ms"
            )
            ...
            if e.result:
                ...
                assert (
                    e.result.risk_score >= 70
                ), f"SecurityException raised but low risk score: {e.result.risk_score}"
                print(
                    f"  ✅ Threat prompt: Risk {e.result.risk_score}/100, {e.result.analysis_time_ms:.1f}ms (blocked)"
                )
            else:
...
def validate_hybrid_mode():
    ...
    try:
        ...
        print(
            f"  ✅ Hybrid analysis: Risk {result.risk_score}/100, {result.analysis_time_ms:.1f}ms"
        )
        ...
        assert health.get("status") in [
            ...
            "warning",
            "error",
        ], f"Invalid health status: {health.get('status')}"
...
def validate_rules():
    ...
    try:
        ...
        if bundled_industry.exists():
            industry_rules = list(bundled_industry.glob("*.yml")) + list(
                bundled_industry.glob("*.yaml")
            )
        ...
        if bundled_compliance.exists():
            compliance_rules = list(bundled_compliance.glob("*.yml")) + list(
                bundled_compliance.glob("*.yaml")
            )
