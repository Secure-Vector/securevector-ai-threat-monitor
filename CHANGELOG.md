...
- **Containment drift** — every proof is diffed against its predecessor, per probe. The obvious regression (a path that was contained now reaches) is reported as critical. The one no other tool reports is `enforcement_weakened`: a path still contained, but no longer by SecureVector. Both proofs read "contained" in that case and the verdict does not move, yet the guarantee has migrated to a network control this policy does not manage and cannot notice changing. Drift against a `degraded` or `error` proof is refused outright rather than computed, because an offline run makes every path look contained and diffing against it would manufacture regressions that never happened.
...
- **Containment attestation export** (JSON / CSV / Markdown) — the proof in a form a security reviewer who was not present can read. Verdict first, then what was **not** tested, then the results, in that order. A `degraded` proof exports as a degraded attestation; there is no code path that turns an inconclusive run into a document that reads like a pass. No format claims a guarantee. CSV carries the caveats as leading comment lines rather than in a companion file, because a CSV that can be opened without its caveats will be.
...
- **Agent Egress page** *(Govern → Policies & Controls)* — three tabs. **Containment Proof** leads, because it is the one screen in this product that reports a fact the operator cannot obtain any other way. It renders the verdict, the drift since the last run, the three-column result table, the coverage gaps, and the attestation export. A `degraded` verdict is styled as inconclusive, never as a pass. **Destinations** is the blast radius, with one-click promotion; hosts blocked by a rule a promotion cannot clear show "policy edit" instead of an Allow button that would silently do nothing. **Policy** carries the preset selector with the counterfactual preview inline, plus the promotion-rate health signal.
