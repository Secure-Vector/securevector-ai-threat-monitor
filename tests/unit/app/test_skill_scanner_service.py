...
class TestFixtureDirectories:
    """Tests using inline tmp_path fixtures (no external fixture dirs needed)."""

    def test_clean_skill_yields_low_risk(self, tmp_path):
        _write(tmp_path, "permissions.yml", "permissions:\n  networks: []\n  files: []\n  env_vars: []\n")
        _write(tmp_path, "main.py", 'print("hello world")\n')
        result = _scan_dir(tmp_path)
        assert result.risk_level in ("LOW", "MEDIUM")

    def test_manifest_skill_has_manifest(self, tmp_path):
        _write(tmp_path, "permissions.yml", "permissions:\n  networks: []\n  files: []\n  env_vars: []\n")
