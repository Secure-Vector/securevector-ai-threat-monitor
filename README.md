...
> - **Lower threat-tab noise** — `/analyze` threat scans now run only on prose-shaped tool inputs (WebFetch / Skill / Task / Agent prompts) and on user prompts. Shell command bodies, file content, and edit diffs are still audited to the SHA-256 hash chain but no longer fed to the LLM-prose rule pack — that mismatch was the noise source. Two over-broad community rules were also tightened (`sv_llm_008_excessive_agency` bounded distance, `sv_community_output_001_credential_leak` excludes path delimiters).
